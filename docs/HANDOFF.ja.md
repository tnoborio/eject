...
[#26](https://github.com/tnoborio/eject/pull/26)をマージ済み。
検証したアプリケーション改訂は`main`の`7398c32ef6a004e552f96cbb5097813c6d64a6a6`。
PR #26は`682685d`から`56c0129`までの独立レビューで承認され、
新規Chromiumでnative fetchの再現確認も行った。マージ後の全4チェックは
[run 35851500917](https://github.com/tnoborio/eject/actions/runs/35851500917)で成功。
一時切り戻し後、本番デプロイ`dpl_Dm4gUqhSpbxGNJRMi8HpriJ1LriN`
（`eject-3kddtye2x-sasara.vercel.app`）を昇格し、
固定URL`https://eject-bice.vercel.app`がこのデプロイを指すことを確認した。

固定URLを新規Chromiumで開き、HTTP 200、未ログイン表示、EJECTボタン無効、
未認証の端末一覧HTTP 401、`DELIVERY_DISABLED`と`ENROLLMENT_DISABLED`を確認。
別の新規ブラウザーセッションでは、本番の画面と通信を差し替えた架空のperson APIを使い、
期限切れからの復帰、更新後の一時停止、OTP誤入力後の再入力を検証した。
この模擬検証でメールは送っておらず、実アカウント・認証サービスのCookie・
2アカウント間の同意操作の証拠とは区別する。

所有者から検証用アドレス2つを受け取り、表示名の選定も任された。既存の有効なアカウントを維持し、
2人目を招待アカウントとして登録した。専用Supabaseプロジェクトは停止していたため再開し、
`ACTIVE_HEALTHY`を確認した。DBの配送ゲートは無効を維持している。新規ブラウザーで1人目の
実OTPログインに成功し、同じchallenge内で誤入力のHTTP 401後、正しいコードでHTTP 204を確認した。
初期の接続数は0、一時停止は無効だった。

2026-09-24に2人目も実メールのOTPでログインに成功した。本番に対する独立したChromiumの
セッションで、接続時に許可が自動付与されないこと、双方からの明示的な許可、一時停止・再開、
許可の取り消し、切断、新しいコードによる再接続で双方の許可が復活しないことを確認した。
1人目は一晩経過後の同意情報取得が最初HTTP 401になり、画面の再読み込みでアクセスが復帰してから
接続の検証を行った。これは本番での復帰の証拠であり、更新処理の同時実行やローテーション内部を
計測したものではない。

検証後は2人の接続を解除し、両ブラウザーセッションをログアウト（HTTP 204）した。
その後の保護された端末一覧取得はHTTP 401。読み取り専用のDB確認では、検証した2人の間の
有効な接続と相互の許可、検証アカウント所有の端末と受信コマンドはすべて0件で、双方とも
一時停止は無効だった。配送は無効を維持した。専用の検証ブラウザーは終了した。
アドレス・コード・Cookie・非公開イベントログはこの文書に記録しない。

次は招待情報を限定的に削除する定期処理を設定し、遠隔配送前にEJECT BACKの期限と遅延結果の
記録を解決する。Windowsの準備状態とクライアント実装も進める。
光学ドライブの実機検証は機材を用意できるまで保留し、配送・端末登録は無効を維持する。
以下の過去記録にあるレビュー・反映・2アカウント検証待ちの記述より、この現在の状態を優先する。

## レビュー追記 — 2026-09-23

ローカル`main`の`fcce1ca`を対象としたコード・MVPレビューを
[レビュー資料](REVIEW-2026-09-23.ja.md)に記録した。フォローアップbranchでは、rejectまたは
一時的に利用不可のOTP検証で元の10分間PKCE challengeをreplacement cookieを発行せずに保持する。
成功時は引き続きchallengeをclearし、HttpOnly sessionを設定する。Web consoleはprotected routeの
...
フォローアップはローカルで`npm run check`、`npm test`（protocol 11件、control-plane 117件）、
...
logout後の遅延protected JSON、新しいlogin後の古いretried 401を対象にする。browser E2E、実OTP、cloud、実機の証拠は実行していない。
...
## A1Xの実機検証基盤 — 2026-09-23

所有者はトレイ式光学ドライブを持っているが、現在A1X用に見つけられないと報告し、既存Orchestrator
Windows workerを利用できることを確認した。A1Xで排出しない限定的な準備診断が終了コード0で成功した。
worker v3、非昇格の一般ユーザー、PowerShell 5.1、.NET SDK 6.0.423で、
現在列挙された光学ドライブは0台。ディスク内容、画面撮影、認証情報、
広範なハードウェア情報は取得していない。同workerではKeystoneのcheck-onlyジョブも
成功したことをジョブのメタデータで確認した。
指定された`~/repos/keystone-call-of-catsh`はこのLinuxホストとWindowsユーザーホームの
対応するパスにはなく、そのリポジトリのCI設定自体は未確認。

物理試験はドライブを見つけて接続できるまで延期する。この所有者用workerは排出しないcheckと、
レビュー済みの自己完結型Windows x64 EJECT成果物の転送、
限定的な検証結果の回収に使う。汎用PowerShell実行機能をEJECTエージェントへ取り込まない。
自己完結型ビルドなら、EJECTの実行だけのためにA1Xへ.NET 10をインストールする必要はない。
ドライブの接続を確認して再列挙した後、ローカルで承認した意図的な単発試行へ進む。
物理的なトレイ開放と機器の対応可否はまだ未検証。

実機準備調査では物理配送を有効化していない。その後のマージ・本番反映の承認と検証は上記に記録した。
...
63. PR #20はrun 30056420951でActions 4 jobとVercel check 2件がmerge前にすべて成功した。
    relationship lifecycleのcheckoutはformat、ESLint、TypeScript、dependency rule、protocol test 11件、
...
PostgreSQL証拠では、`eject_test`という名前のephemeral PostgreSQL 17 databaseへ
`TEST_DATABASE_URL`を設定し、`npm run test:postgres --workspace @eject/control-plane`を実行して
