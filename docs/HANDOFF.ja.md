...
[レビュー資料](REVIEW-2026-09-23.ja.md)に記録した。静的チェック、プロトコル11件、
サーバー110件、本番ビルドが成功し、架空入力でOTP再入力の問題を再現した。
クラウド・実機の証拠は更新しておらず、製品コード・配送ゲートも変更していない。

予定していたブラウザー検証の前にOTP再入力とセッション更新を修正する。
遠隔配送前にEJECT BACKの期限と遅延結果の記録を解決し、Windowsクライアントに加え、
端末の準備完了・接続状態の遷移を完成させる。実機の証拠は引き続き必須。
以下のスナップショットは7月のデプロイ記録であり、9月の本番再確認ではない。
